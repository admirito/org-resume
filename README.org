...
This repository is a fork of Xavier Garrido's [[https://github.com/xgarrido/org-resume][org-resume]] project which
I have modified it for my personal use. As I have removed his name
from the resume, I thought I should appreciate his excellent work,
here.

I have also made small changes in the appearance and the building
process to make it better for my own use case.

The Xavier's project doesn't have a LICENSE yet, but I have asked him
personally and he answered me the same as [[https://github.com/xgarrido/org-resume/issues/1][this one]]. So, for now, I am
...
#+BEGIN_SRC shell :tangle no :results output
  if [ -d .git ]; then
      LC_MESSAGES=en git --no-pager log -1 HEAD --date=short --pretty=format:'<i class=\"fab fa-github-alt\"></i> commit <a href=https://github.com/admirito/org-resume/commit/%H>%h</a> - %ad'
  fi
#+END_SRC
...
#+BEGIN_SRC shell :tangle no :results output
  ...
  preamble+='<span class=\"name\">Mohammad Razavi</span><br>'
  preamble+='Software Engineer<br>'
  preamble+='Berlin, Germany<br>'
  preamble+='</div>'
  preamble+='<div id=\"contact\">'
  preamble+='<span class=\"name\">&nbsp;</span><br>'
  preamble+='Telegram: <a href=\"https://t.me/admirito\">@admirito</a><br>'
  preamble+='LinkedIn: <a href=\"https://www.linkedin.com/in/admirito/\">admirito</a><br>'
  preamble+='GitHub: <a href=\"https://github.com/admirito\">admirito</a><br>'
  preamble+='mrazavi64 at gmail<br>'
  preamble+='<a href=\"https://admirito.github.io/org-resume\">admirito.github.io/org-resume</a><br>'
  preamble+='</div>'
  echo -ne $preamble
#+END_SRC
*** Project definition
...
#+BEGIN_SRC latex
  ...
  \def\myweb{admirito.github.io/org-resume}
  \def\mytelegram{@admirito}
  \def\mytelegramlink{https://t.me/admirito}
  \def\mylinkedin{admirito}
...
#+BEGIN_SRC latex
  ...
  \renewcommand{\mytitle}{\normalsize\@author\\\footnotesize Software Engineer
        \vspace{+0.05in} \\ Berlin, \\ \vspace{-0.05in} Germany}
  }
  \renewcommand*{\maketitle}{%
    ...
    \begin{minipage}[t]{1.7in}
      \ifthenelse{\boolean{@fa}}{
        \flushright \footnotesize تلگرام:~{\href{\mytelegramlink}{\mytelegram}} \\
      }{
        \flushright \footnotesize Telegram:~{\href{\mytelegramlink}{\mytelegram}} \\
      }
...
#+BEGIN_SRC sh :results output :tangle no
  if [ -d .git ]; then
      log=$(LC_MESSAGES=en git --no-pager log -1 HEAD --date=short --pretty=format:'commit \\href{https://github.com/admirito/org-resume/commit/%H}{\\color{gray!50}\\texttt{%h}} -- %ad')
